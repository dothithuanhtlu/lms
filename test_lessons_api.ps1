    ...
    try {
        ...
        if ($response) {
            ...
            if ($response.lessons -or $response.lessonInfos -or $response.title) {
                ...
                Write-Host "⚠️  No lesson data detected in response" -ForegroundColor Yellow
            }
            ...
            Write-Host "⚠️  Empty response received" -ForegroundColor Yellow
        }
...
Write-Host "`n🔍 Checking if LMS server is running..." -ForegroundColor Yellow
...
Write-Host "`n📚 Available Lesson Endpoints:" -ForegroundColor Blue
...
Write-Host "`n🚀 To restart the application, run:" -ForegroundColor Yellow
